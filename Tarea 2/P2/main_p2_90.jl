...
function UnitCommitmentFunction(Data)
    ...
    #PARA TENER 8 en p2c
    @constraint(model,reservaup[t in 1:T],sum(r[gen,t] for gen in GeneratorSet)>=reserva_90_of[t])
    #PARA TENER 87
    #@constraint(model,reservaup[t in 1:T],sum(r[gen,t] for gen in GeneratorSet)>=2*reserva_90_of_rial_otros[t])
...
data_x = DataFrame(i=Int[], t=Int[], x=Float64[])
data_u = DataFrame(i=Int[], t=Int[], u=Float64[])
data_v = DataFrame(i=Int[], t=Int[], v=Float64[])
